...
// アプリケーションのタイトル名
static const TCHAR szClassName[] = TEXT("骨格座標を深度座標に合わせるサンプル");
...
k4a_result_t CreateKinect( const char* pRecordingFileName )
{
	...
	if ( hr == K4A_RESULT_SUCCEEDED )
	{
		...
		if ( pRecordingFileName )
		{
			...
			{
				MessageBox( NULL, TEXT("Azure Kinect ファイル再生を開始できませんでした"), TEXT("エラー"), MB_OK );
			}
		}
		else
		{
	...
	{
		MessageBox( NULL, TEXT("Azure Kinect の初期化に失敗 - レコーディングの状態を確認してください"), TEXT("エラー"), MB_OK );
	}
	return hr;
}
